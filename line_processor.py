...
class LineProcessor:
    ...
    def tf_function(self, vec: np.ndarray) -> np.ndarray:
        vec[0] = max(min(vec[0], 319), 0.0)
        vec[0] = max(min(vec[0], 239), 0.0)
    ...
    def publish_road(self):
        ...
        y = (segments[0][1][1] + segments[-1][1][1]) / 2.0
        ...
        for idx, (start, end, _, width) in enumerate(segments):
            ...
            center_pos = [(start[0] + end[0]) / 2.0, y]
            length = math.sqrt(pow(start[0] - end[0], 2))
    ...
    def realtime_line_segmentation(self, tf_function, image: np.ndarray, hsv: np.ndarray):
        ...
        bin = cv2.rectangle(bin, (0, 120+30), (320, 240), 0, -1)
        ...
        if self.dist(box[0], box[1]) > self.dist(box[1], box[2]):
        ...
        wall_height = self.dist(*walls[0])
        wall_width = self.dist(*between[0])
        ...
        for x in space_x:
            ...
            point = bin[np.clip(int(coordinates[1]), 0, hsv.shape[0] - 1), np.clip(int(coordinates[0]), 0, hsv.shape[1] - 1)]
            if point > 127:
                c += 1
                xs.append(x)
        ...
        if self.start is None:
            start_vec = np.array([0, wall_height])
            frame_start_vec = (T_inv @ start_vec.T).T + corner
            print(frame_start_vec)
            ...
            if len(self.segments) > 0:
                dst = self.dist(self.segments[-1][1], map_end_vec)
                is_allowed = (dst >= 0.5)
            else:
                dst = self.dist(self.start, map_end_vec)
                is_allowed = (dst >= 0.5)
